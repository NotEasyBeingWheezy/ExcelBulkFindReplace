...
def process_sheet_with_rules(sheet, rules, max_rows_to_process=300):
    ...
    OPTIMIZATION: Groups rules by column pair; reads only the needed columns
    per group rather than the entire used range.
    """
    try:
        print(f"      Analyzing sheet structure")

        # Get used range to determine row bounds only
        try:
            ...
            first_row = used_range.row  # 1-based sheet row where data starts
            last_row = first_row + rows_to_process - 1
        ...
        for (search_col, update_col), rule_group in grouped_rules.items():
            ...
            # Read only the two columns needed — far faster than reading the whole sheet
            raw_search = sheet.range(f"{search_col}{first_row}:{search_col}{last_row}").value
            search_data = raw_search if isinstance(raw_search, list) else [raw_search]

            if update_col == search_col:
                update_data = search_data
            else:
                raw_update = sheet.range(f"{update_col}{first_row}:{update_col}{last_row}").value
                update_data = raw_update if isinstance(raw_update, list) else [raw_update]

            # SINGLE PASS through all rows for this column pair
            for row_idx in range(rows_to_process):
                try:
                    search_cell_value = search_data[row_idx]
                    ...
                    current_value = update_data[row_idx]
                    current_value_str = str(current_value) if current_value is not None else ""

                    # Check if update is needed
                    if current_value_str.strip() != target_value.strip():
                        actual_row = first_row + row_idx
                        sheet.range(f"{update_col}{actual_row}").value = target_value
                        update_details[rule_name] += 1
                        total_updates += 1
                        all_affected_rows.add(actual_row)
                    ...
                    logging.debug(f"Skipped cell at row {first_row + row_idx}, column {search_col}", exc_info=True)
                    continue
